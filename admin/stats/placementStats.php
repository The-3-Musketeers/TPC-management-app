...
$placedQuery = "SELECT D.degree_name AS degree_name, B.branch_name AS branch_name, "
			...
			."FROM (SELECT db_id, 0 AS placed FROM degree_branch WHERE db_id NOT IN (SELECT db_id FROM students_data WHERE job_offers!='') "
			."UNION "
			."SELECT db_id, COUNT(roll_number) AS placed FROM students_data WHERE job_offers!='' GROUP BY db_id) AS placed_table, "
			."(SELECT db_id, 0 AS remaining FROM degree_branch WHERE db_id NOT IN (SELECT db_id FROM students_data WHERE job_offers IS NULL OR job_offers='') "
			."UNION "
